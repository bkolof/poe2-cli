...
-- The trade site stat for a text like "movement speed": a pseudo total when
-- there is one, else an explicit mod; the shortest match is the most specific.
local function findTradeStat(query)
	local best

	for _, category in ipairs(tradeHelpers.getTradeStats()) do
		local rank = ({ pseudo = 0, explicit = 1 })[category.id]

		for _, entry in ipairs(rank and category.entries or {}) do
			local better = not best or rank < best.rank or (rank == best.rank and #entry.text < #best.text)

			if contains(entry.text, query) and better then
				best = { id = entry.id, text = entry.text, rank = rank }
			end
		end
	end

	if not best then
		error("no trade site stat matches '" .. query .. "'", 0)
	end
...
-- build. Prices are capped in exalted orb equivalents.
function poe2.tradeQuery(request)
	local name, slot = findSlot(request.slot)

	if name:find("Jewel") then
		error("trade searches do not support jewel sockets yet", 0)
	end
	...
	for _, requirement in ipairs(request.require or {}) do
		local stat = findTradeStat(requirement.stat)
		table.insert(required, { tradeId = stat.id, value = requirement.min, text = stat.text })
	end
	...
	for _, requirement in ipairs(required) do
		table.insert(requiredList, { id = requirement.tradeId, text = requirement.text, min = requirement.value })
	end

	return { slot = name, query = generated.json, weights = weightList, required = requiredList }
end
